...
impl Gemma3RMSNorm {
    pub fn load_from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let weight = ct
            .tensor(reader, weight_name, &Device::Cpu)?
    ...
    pub fn forward(
        ...
        let norm_hidden_states = (hidden_states.sqr()?.sum_keepdim(D::Minus1)?
            / hidden_size as f64)?;
        let hidden_states_normed = hidden_states.broadcast_div(
            &(norm_hidden_states + self.epsilon as f64)?.sqrt()?,
        )?;

        let output = hidden_states_normed
...
impl Gemma3Attention {
    ...
    fn create_causal_mask(
        ...
        let mask_tensor =
            Tensor::from_slice(&mask, (seq_len, seq_len), &Device::Cpu)?;
        let expanded_mask =
            mask_tensor.expand(&[batch_size, dim, seq_len, seq_len])?;

        let zeros = Tensor::zeros(
            (batch_size, dim, seq_len, seq_len),
            dtype,
            &Device::Cpu,
        )?;
        let negatives = Tensor::full(
            min_value,
            (batch_size, dim, seq_len, seq_len),
            &Device::Cpu,
        )?
    ...
    pub fn forward(
        ...
        let attention_bias = match attention_bias {
            Some(attention_bias) => {
                let (batch_size, dim, seq_length, _) =
                    attention_bias.shape().dims4()?;
        ...
        let context_layer =
            context_layer.transpose(1, 2)?.flatten_from(D::Minus2)?;
...
impl Gemma3Layer {
    ...
    pub fn forward(
        ...
        let (hidden_states, _) =
            self.post_attention_layernorm.forward(&hidden_states, None)?;
        let hidden_states = residual.broadcast_add(&hidden_states)?;

        let residual = hidden_states.clone();
        let (hidden_states, _) =
            self.pre_feedforward_layernorm.forward(&hidden_states, None)?;
        let hidden_states = self.mlp.forward(&hidden_states)?;

        let (hidden_states, _) =
            self.post_feedforward_layernorm.forward(&hidden_states, None)?;
...
impl Gemma3Model {
    pub fn load<R: std::io::Seek + std::io::Read>(
        ...
        let pool = match model_type {
            ModelType::Classifier => {
                candle_core::bail!(
                    "`classifier` model type is not supported for Gemma3"
                )
            }
        ...
        let layers = (0..config.num_hidden_layers)
            .map(|layer_idx| {
                let attention_type = if (layer_idx + 1)
        ...
        let dense1_weight = vb_dense1.pp("linear").get(
            (4 * config.hidden_size, config.hidden_size),
            "weight",
        )?;
        let dense1 = Linear::new(dense1_weight, None, None);

        let dense2_weight = vb_dense2.pp("linear").get(
    ...
    pub fn forward(&self, batch: Batch) -> candle_core::Result<Option<Tensor>> {
        ...
        let (input_ids, position_ids, input_lengths, attention_bias) =
            if batch_size > 1 {
                ...
                let position_ids =
                    Tensor::from_vec(position_ids, shape, &Device::Cpu)?;
        ...
        let cos_local = self
            ...
            .index_select(&position_ids.flatten_all()?, 0)?;
        let cos_local =
            cos_local.reshape((batch_size, 1, max_length, self.rotary_dim))?;
        let sin_local = self
            .rotary_cache_local_attention
            .1
            .index_select(&position_ids.flatten_all()?, 0)?;
        let sin_local =
            sin_local.reshape((batch_size, 1, max_length, self.rotary_dim))?;
        ...
        let dense1_hidden = self.dense1.forward(
            pooled_embeddings.as_ref().ok_or_else(|| {
