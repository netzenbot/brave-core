...
impl Gemma3Embedder {
    pub fn new(
        ...
        let content = gguf_file::Content::read(&mut cursor).map_err(|e| {
            EmbedderError::ModelError(format!("GGUF parse error: {e}"))
        })?;
    ...
    pub fn embed(&self, text: &str) -> Result<Vec<f32>, EmbedderError> {
        ...
        let normed_output = normed_output
            .ok_or_else(|| {
                EmbedderError::ModelError("Embedding generation failed!".to_string())
            })?;
    ...
    fn encode_batch(&self, texts: &[String]) -> Result<Batch, EmbedderError> {
        ...
        for text in texts {
            let encoding = self.tokenizer.encode(text.as_str(), true).map_err(
                |e| EmbedderError::TokenizerError(format!("Tokenization failed: {e}")),
            )?;
