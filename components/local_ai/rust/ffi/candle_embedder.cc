...
void CandleEmbedder::Create(std::vector<uint8_t> weights,
  ...
  embedder_ptr->rust_thread_.task_runner()->PostTask(
      ...
      base::BindOnce(&CandleEmbedder::CreateOnRustThread, embedder_ptr,
                     ...
                     std::move(config), std::move(callback), origin_task_runner));
...
void CandleEmbedder::CreateOnRustThread(
  ...
  auto* callback_data = new InitCallbackData{
      wrapper, std::move(callback), origin_task_runner};

  candle_embedder_new(
      weights.data(), weights.size(), weights_dense1.data(),
      weights_dense1.size(), weights_dense2.data(), weights_dense2.size(),
      tokenizer.data(), tokenizer.size(), config.data(), config.size(),
...
void CandleEmbedder::OnInitCallback(void* user_data,
  ...
  if (success) {
    ...
    auto wrapper = std::unique_ptr<CandleEmbedder>(callback_data->wrapper);
    callback_data->origin_task_runner->PostTask(
        FROM_HERE,
        base::BindOnce(std::move(callback_data->callback), std::move(wrapper),
                       std::string()));
  } else {
    std::string error_message = data ? std::string(data) : "Unknown error";
    callback_data->origin_task_runner->PostTask(
        FROM_HERE,
        base::BindOnce(std::move(callback_data->callback), nullptr,
                       error_message));
...
void CandleEmbedder::Embed(const std::string& text, EmbedCallback callback) {
  ...
  rust_thread_.task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&CandleEmbedder::EmbedOnRustThread,
