...
pub type EmbedCallback =
    extern "C" fn(*mut c_void, *const f64, size_t);
...
pub extern "C" fn candle_embedder_new(
    ...
    let result = panic::catch_unwind(|| {
        let weights_slice =
            unsafe { slice::from_raw_parts(weights, weights_len) };
        let weights_dense1_slice = unsafe {
            slice::from_raw_parts(weights_dense1, weights_dense1_len)
        };
        let weights_dense2_slice = unsafe {
            slice::from_raw_parts(weights_dense2, weights_dense2_len)
        };
        let tokenizer_slice =
            unsafe { slice::from_raw_parts(tokenizer, tokenizer_len) };
    ...
    match result {
        Ok(Ok(embedder)) => {
            let boxed = Box::new(CCandleEmbedder {
        ...
        Err(_) => {
            let c_error = std::ffi::CString::new("Panic during initialization")
                .unwrap();
            init_cb(user_data, false, c_error.as_ptr());
        }
    }
...
pub extern "C" fn candle_embedder_embed(
    ...
    match result {
        Ok(Ok(embedding)) => {
            let f64_embedding: Vec<f64> =
                embedding.iter().map(|&x| x as f64).collect();
            embed_cb(
                user_data,
                f64_embedding.as_ptr(),
                f64_embedding.len(),
            );
        }
        Ok(Err(_)) | Err(_) => {
...
#[no_mangle]
pub extern "C" fn candle_embedder_destroy(
    embedder: *mut CCandleEmbedder,
) {
    if !embedder.is_null() {
